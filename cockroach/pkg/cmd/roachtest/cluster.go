...
func findBinaryOrLibrary(binOrLib string, name string) (string, error) {
	...
	if err != nil {
		...
		dirs := []string{
			filepath.Join(gopath, "/src/sqlfmt/cockroach/"),
			filepath.Join(gopath, "/src/sqlfmt/cockroach", binOrLib+suffix),
			filepath.Join(os.ExpandEnv("$PWD"), binOrLib+suffix),
			filepath.Join(gopath, "/src/sqlfmt/cockroach", binOrLib),
		}
...
func (f *clusterFactory) newCluster(
	...
	for i := 1; ; i++ {
		c := &clusterImpl{
			...
			// the loop. See:
			//
			// https://sqlfmt/cockroach/issues/67906#issuecomment-887477675
			name:           f.genName(cfg),
			spec:           cfg.spec,
...
func (c *clusterImpl) CheckReplicaDivergenceOnDB(
	...
	// see:
	// https://sqlfmt/cockroach/pull/34520
	//
